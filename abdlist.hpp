...
template<typename ValueType, size_t SBufLen = 0> class ABDList
{
    ...
    public:
        ABDList(size_t nCap = 0)
            ...
            , m_Free1(0)
        {
            if(nCap > std::extent<decltype(m_SBuf)>::value){
                m_DBuf.resize(nCap - std::extent<decltype(m_SBuf)>::value);
            }
            Reset();
        }
    ...
    public:
        ...
        {
            return std::extent<decltype(m_SBuf)>::value + m_DBuf.size();
        }
    ...
    private:
        ...
        {
            if(nPos < std::extent<decltype(m_SBuf)>::value){
                return m_SBuf[nPos];
            }
            return m_DBuf[nPos - std::extent<decltype(m_SBuf)>::value];
        }
